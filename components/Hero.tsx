...
export default function Hero({ c, lang }: Props) {
  ...
  return (
    <>
      <section style={{ padding: isMobile ? '56px 20px 40px' : '96px 48px 72px', position: 'relative', overflow: 'hidden' }}>
        <div style={{
          ...
          <div>
            <div style={{
              ...
              textTransform: 'uppercase', color: th.muted, marginBottom: 20,
            ...
            <h1 style={{
              ...
              fontSize: isMobile ? 'clamp(52px, 14vw, 80px)' : 'clamp(64px, 8vw, 128px)',
              lineHeight: 0.95, letterSpacing: -2,
              margin: '0 0 24px', fontWeight: 400, color: th.ink,
            }}>
              {c.hero.title[0]}<br />
            ...
            <p style={{
              fontSize: isMobile ? 16 : 19, lineHeight: 1.55, color: th.inkSoft,
          ...
          {!isMobile ? (
            ...
            <div style={{ display: 'flex', gap: 12, justifyContent: 'center' }}>
              {ADVISORS.map((a, i) => (
                <div key={a.id} style={{
                  width: 140, height: 180, flexShrink: 0,
                  background: i === 0 ? ADVISORS[1].tintLight : ADVISORS[0].tintLight,
                  border: `1px solid ${th.border}`,
                  padding: 8, position: 'relative',
                  transform: i === 0 ? 'rotate(-1.5deg)' : 'rotate(1.5deg)',
                  marginTop: i === 1 ? 16 : 0,
                }}>
                  <Image src={a.photo} alt={a.name} fill style={{ objectFit: 'contain', objectPosition: 'center bottom', padding: 4 }} sizes="140px" />
                  <div style={{ position: 'absolute', bottom: 6, left: 8, fontFamily: th.fontDisplay, fontSize: 13, fontStyle: 'italic', color: '#fff', textShadow: '0 1px 6px rgba(0,0,0,0.5)', zIndex: 1 }}>
                    {i === 0 ? (lang === 'es' ? 'Nueva York' : 'New York') : 'Chicago'}
                  </div>
                </div>
              ))}
            </div>
      ...
      {/* Trust bar */}
      <section style={{ padding: isMobile ? '16px 20px 40px' : '24px 48px 72px', borderBottom: `1px solid ${th.border}` }}>
        <div style={{
          maxWidth: 1280, margin: '0 auto',
          ...
          <div style={{ display: 'flex', gap: isMobile ? 20 : 36, flexWrap: 'wrap', flex: 1 }}>
            {TRUSTED_COMPANIES.map(co => (
              <span key={co} style={{
                fontFamily: th.fontDisplay, fontSize: isMobile ? 18 : 22, color: th.inkSoft,
...
function AdvisorPhoto({ src, name, city, bg, rotate, offsetTop }: {
  ...
  return (
    <div style={{
      width: 280, height: 340, background: bg,
      border: `1px solid ${th.border}`,
      ...
      <Image src={src} alt={name} fill style={{ objectFit: 'contain', objectPosition: 'center bottom', padding: 8 }} sizes="280px" />
      <div style={{
        position: 'absolute', bottom: 10, left: 14,
        fontFamily: th.fontDisplay, fontSize: 20, fontStyle: 'italic',
        color: '#fff', textShadow: '0 1px 8px rgba(0,0,0,0.5)', zIndex: 1,
      }}>{city}</div>
    </div>
  );
}
