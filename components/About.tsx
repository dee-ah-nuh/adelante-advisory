...
export default function About({ c, lang }: Props) {
  ...
  return (
    <section id="about" style={{ padding: isMobile ? '64px 20px' : '120px 48px', background: th.ink, color: th.bg }}>
      <div style={{ maxWidth: 1280, margin: '0 auto' }}>
        ...
        <div style={{
          ...
          gap: isMobile ? 24 : 80,
          alignItems: 'start', marginBottom: isMobile ? 40 : 80,
        }}>
          <h2 style={{
            fontFamily: th.fontDisplay,
            fontSize: isMobile ? 48 : 72,
            lineHeight: 0.98, letterSpacing: -2, fontWeight: 400,
          }}>{c.about.title}</h2>
          <p style={{
            fontSize: isMobile ? 16 : 20, lineHeight: 1.55, color: th.bg,
            opacity: 0.85, paddingTop: isMobile ? 0 : 12,
          }}>{c.about.intro}</p>
        </div>
        ...
        <div style={{
          ...
          {advisors.map(({ data, advisor }) => (
            <FounderCard key={advisor.id} data={data} advisor={advisor} lang={lang} isMobile={isMobile} />
          ))}
        </div>
      </div>
    </section>
...
function FounderCard({
  ...
  return (
    <div style={{
      ...
      <div style={{
        width: '100%', height: isMobile ? 240 : 320, marginBottom: 24,
      ...
      <h3 style={{
        fontFamily: th.fontDisplay, fontSize: isMobile ? 28 : 36, fontWeight: 400,
