...
export default function Booking({ c, lang }: Props) {
  ...
  return (
    <section id="booking" style={{ padding: isMobile ? '64px 20px' : '120px 48px' }}>
      <div style={{ maxWidth: 960, margin: '0 auto' }}>
        <div style={{ textAlign: 'center', marginBottom: isMobile ? 40 : 56 }}>
          ...
          <h2 style={{
            ...
            lineHeight: 0.98, letterSpacing: -2, fontWeight: 400, marginBottom: 16,
          }}>{c.booking.title}</h2>
          <p style={{ fontSize: isMobile ? 16 : 18, color: th.inkSoft, fontStyle: 'italic', fontFamily: th.fontDisplay }}>
            {c.booking.sub}
          </p>
        </div>
...
function AdvisorBookingCard({ advisor, lang }: { advisor: typeof ADVISORS[number]; lang: 'en' | 'es' }) {
    ...
    <div style={{
      ...
      <div style={{ display: 'flex', gap: 16, alignItems: 'center' }}>
        <div style={{
          ...
          fontFamily: th.fontDisplay, fontSize: 20, fontWeight: 500, flexShrink: 0,
        }}>{advisor.initials}</div>
        <div>
          <div style={{ fontFamily: th.fontDisplay, fontSize: 22, letterSpacing: -0.3 }}>{advisor.name}</div>
          <div style={{ fontSize: 13, color: th.muted }}>{lang === 'es' ? advisor.companyEs : advisor.companyEn}</div>
        </div>
      </div>
      <div style={{ fontSize: 12, color: th.muted, fontFamily: th.fontMono, letterSpacing: 0.5, paddingTop: 4, borderTop: `1px solid ${th.border}` }}>
        {lang === 'es' ? advisor.specialtyEs : advisor.specialtyEn}
      </div>
      <a href={calendlyUrl} target="_blank" rel="noreferrer" style={{
        ...
        textDecoration: 'none', letterSpacing: 0.2, marginTop: 4,
      }}>{label} →</a>
