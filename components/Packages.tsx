...
export default function Packages({ c, lang }: Props) {
  return (
    <section id="packages" style={{ padding: '120px 48px', background: th.bgAlt }}>
      <div style={{ maxWidth: 1280, margin: '0 auto' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 20 }}>
          {c.packages.items.map((pkg, i) => (
            <PackageCard key={i} pkg={pkg} lang={lang} cta={c.packages.cta} />
          ))}
        </div>

        {/* Contact options */}
        <div style={{
          marginTop: 48, paddingTop: 40, borderTop: `1px solid ${th.border}`,
...
function PackageCard({ pkg, lang, cta }: {
    ...
    <div style={{
      ...
      {featured && (
        <div style={{
          ...
          fontFamily: th.fontMono, textTransform: 'uppercase',
          borderRadius: 2,
      ...
      <div style={{
        ...
        color: featured ? 'rgba(255,255,255,0.6)' : th.muted,
        marginBottom: 4, textTransform: 'uppercase',
      }}>{pkg.sessions}</div>
      <div style={{
        fontSize: 14, fontStyle: 'italic', fontFamily: th.fontDisplay,
        color: featured ? th.bg : th.accent, marginBottom: 24,
      }}>{pkg.tag}</div>
      ...
      <ul style={{ listStyle: 'none', padding: 0, marginBottom: 28, flex: 1 }}>
        {pkg.bullets.map((b, j) => (
          <li key={j} style={{
            ...
            color: featured ? 'rgba(255,255,255,0.85)' : th.inkSoft,
          }}>
